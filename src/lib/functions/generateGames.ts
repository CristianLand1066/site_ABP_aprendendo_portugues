...
export function generateMemoryPairs() {
  const shuffled = shuffleArray(
    getExportedLists().memoryPairsList as { word: string; image?: string }[]
  );
  const count = 12;
...
export function buildDominoPieces(
  syllables: string[]
...
export function buildPortugueseLetters(): string[] {
  return BASE_LETTERS_NUMBERS.map((letter) => letter);
}
